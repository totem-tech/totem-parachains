...
// TODO Fix compiler warning on Perquintill
use sp_runtime::{
	traits::{
	...
	FixedPointOperand, 
	FixedU128, 
	// Perquintill, // Totem: Commented in case of repurcussions
	RuntimeDebug,
};
use sp_std::prelude::*;
...
// Note that the MultiplierUpdate and TargetedFeeAdjustment come from the original crate and not this one. That is why the relevant code is commented below.
...
mod tests {
	...
	use pallet_balances_totem::Call as BalancesCall;
	...
	impl OnUnbalanced<pallet_balances_totem::NegativeImbalance<Runtime>> for DealWithFees {
		fn on_unbalanceds<B>(
			mut fees_then_tips: impl Iterator<Item = pallet_balances_totem::NegativeImbalance<Runtime>>,
		) {
	...
	impl ExtBuilder {
		...
		pub fn build(self) -> sp_io::TestExternalities {
			...
			pallet_balances_totem::GenesisConfig::<Runtime> {
		...
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				System::assert_has_event(RuntimeEvent::Balances(
					pallet_balances_totem::Event::Transfer { from: 2, to: 3, amount: 80 },
				));
				// Killed Event
