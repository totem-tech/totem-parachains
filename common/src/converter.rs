...
impl Convert<u64, u32> for Converter {
...
impl Convert<u64, u128> for Converter {
...
impl TryConvert<u64, u32> for Converter {
	fn try_convert(x: u64) -> Option<u32> {
		if x <= u32::MAX as u64 {
			Some(x as u32)
		} else {
			None
...
impl TryConvert<i128, u64> for Converter {
	fn try_convert(x: i128) -> Option<u64> {
		if x >= 0 {
			u64::try_from(x).ok()
		} else {
			None
